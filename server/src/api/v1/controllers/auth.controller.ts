...
const setAuthCookies = (res: Response, accessToken: string, refreshToken: string) => {
  ...
  res.cookie("accessToken", accessToken, { ...options, maxAge: authService.ACCESS_TOKEN_MAX_AGE });
  res.cookie("refreshToken", refreshToken, { ...options, maxAge: authService.REFRESH_TOKEN_MAX_AGE });
};
...
export const checkEmailOtp = async (req: Request, res: Response): Promise<void> => {
  const { email, otp } = req.body;
  const { accessToken, refreshToken } = await authService.verifyRegisterOtp(email, otp);
...
export const resetPassword = async (req: Request, res: Response): Promise<void> => {
  ...
  const { accessToken, refreshToken } = await authService.resetUserPassword(email, resetToken, newPassword);
  setAuthCookies(res, accessToken, refreshToken);
